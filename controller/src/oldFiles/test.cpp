...
double TrajectoryController::longitudinalController()
{
    ...
    if(m_useBackwardMotion)
    {
        computePathDirection(m_linearVel__m_s);
        ROS_WARN("VELOCITY IS %f ",m_linearVel__m_s);
    }
...
void TrajectoryController::localisation()
{
    ...
    ROS_INFO("Orientation error bfore %f",m_orientationError );
    if(m_reverseMode)
    {
        if(m_orientationError < 0.0){
            m_orientationError = 3.14 + m_orientationError;
        }
        else{
        m_orientationError = 3.14 - m_orientationError;
        }
        ROS_INFO("Orientation error  inside %f",m_orientationError );
    }
    ...
    ROS_ERROR("CURVATURE %f",m_curvature);
...
void TrajectoryController::computePathDirection(double &linearVel)
{
